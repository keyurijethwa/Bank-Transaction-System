...
async function createTransaction(req, res) {
  ...
  if (!fromAccount || !toAccount || !amount || !idempotencyKey) {
    return res.status(400).json({
      message:
        "From account, to account, amount and idempotency key are required to create a transaction",
    });
  }
  ...
  if (
    ...
    return res.status(400).json({
      message:
        "Both from account and to account must be active to process a transaction",
    });
  }
  /**
   * 4. Derive sender balance from ledger
